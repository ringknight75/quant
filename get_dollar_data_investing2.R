...
for(i in 1:nrow(join_cal_2_dt)) {
  join_cal_2_dt[i,7] <- join_cal_2_dt %>% 
    ...
    select(dollar_gap) %>% 
    summarise(mean_52week_gap = (max(dollar_gap) + min(dollar_gap)) / 2) %>% unlist()
}
...
for(i in 1:nrow(join_cal_2_dt)) {
  join_cal_2_dt[i,8] <- join_cal_2_dt %>% 
    filter(date <=  unlist(join_cal_2_dt[i,1]),  date >= unlist(join_cal_2_dt[i,1] - 365)) %>% 
    select(price) %>% 
    summarise(mean_price = (max(price) + min(price)) / 2) %>% unlist()
}
...
for(i in 1:nrow(join_cal_2_dt)) {
  join_cal_2_dt[i,9] <- join_cal_2_dt %>% 
    filter(date <=  unlist(join_cal_2_dt[i,1]),  date >= unlist(join_cal_2_dt[i,1] - 365)) %>% 
    select(index) %>% 
    summarise(mean_index = (max(index) + min(index)) / 2 ) %>% unlist()
}
